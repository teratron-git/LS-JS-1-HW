...
function calculator(number = 0) {
    ...
    return {
        sum: (...allArguments) => allArguments.reduce((previous, current) => previous + current, number),
        dif: (...allArguments) => allArguments.reduce((previous, current) => previous - current, number),
        div: (...allArguments) => allArguments.reduce((previous, current) => {
            if (current === 0) {
        ...
        mul: (...allArguments) => allArguments.reduce((previous, current) => previous * current, number)
    }
}
